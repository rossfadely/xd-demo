...
def XDGMM(datafile, n_components, batch_size, savefile='xd_parms.hdf5',
    ...
    Parameters
    ----------
    datafile:         Location of data file on disk
    n_components:     Number of mixture components
    batch_size:       Mini-batch size during inference
    savefile:         Location to save model parameters and Log likelihoods
    n_iter:           Max number of iterations to run for
    tol:              Relative fractional change in validation likelihood
                      used for stopping
    Nthreads:         Number of threads to use
    R:                Transformation matrix, not implemented
    Ncheck:           Check for convergence every Ncheck iters
    valid_break:      Number of bad validation likelihood steps to stop in
    init_Nbatch:      Mini-batch size for initialization
    wait_epoch:       Delay factor in weight decay
    eta:              Power in weight decay
    nu:               Normalization of weights
    init_n_iter:      Number of iterations in initialization
    w:                Minimum variance in model covariances
    model_parm_file:  Existing model parameter file
    fixed_means:      List of means to fix for each component
    aligned_covs:     List of components to force axis alignment
    ...
    if Xvalid is None:
        Xvalid, Xvalidcov = batch_itr.get_batch()
...
def initialize(batch_itr, model, init_Nbatch, batch_size, fixed_means,
    ...
    if (fixed_means == None) & (aligned_covs == None):
        gmm = GMM(model.n_components, covariance_type='full')
        gmm.fit(X)
    else:
...
class xd_model(object):
    ...
    def sample(self, alpha=None, mu=None, V=None, size=1):
        if alpha is None:
            alpha = self.alpha
            mu = self.mu
            V = self.V
